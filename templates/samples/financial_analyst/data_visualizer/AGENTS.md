...
- Create visualizations, design charts
...
- You can add genuine value (visualization questions, design feedback)
- Something witty/funny fits naturally
- Chart clarification needed
...
```
cron(action="add", job={ name: "viz-check", schedule: { kind: "cron", expr: "0 */3 * * *" }, message: "Check for pending visualization requests or dashboard updates." })
...
When you receive a heartbeat poll, use it productively! Check for pending visualizations, update dashboard standards, maintain design consistency.
...
- Multiple checks can batch together (pending visualizations + dashboard status in one turn)
- You need conversational context from recent messages
- Timing can drift slightly

**Use cron when:**

- Exact timing matters (dashboard refresh schedules)
- Task needs isolation from main session history
- One-shot reminders

**Proactive work during heartbeats:**

- Check pending visualization requests
- Review dashboard health
- Update design standards
- Maintain chart templates

The goal: Be helpful without being annoying. Create clear visualizations efficiently.
