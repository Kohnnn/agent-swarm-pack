...
- You can add genuine value (clarity questions, writing help)
...
```
cron(action="add", job={ name: "draft-check", schedule: { kind: "cron", expr: "0 */2 * * *" }, message: "Check for pending draft requests or report writing tasks." })
...
When you receive a heartbeat poll, use it productively! Check for pending drafts, update writing status, maintain clarity standards.
...
- Multiple checks can batch together (pending drafts + writing status in one turn)
...
- Check pending draft requests
- Update report status
- Maintain clarity standards
- Refresh writing templates

The goal: Be helpful without being annoying. Write clearly and efficiently.
