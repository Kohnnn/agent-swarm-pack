...
- Build models, run analysis
- Work within this workspace

**Ask first:**

- Anything that leaves the machine
- Sharing model outputs externally
...
- You can add genuine value (statistical insights)
...
```
cron(action="add", job={ name: "analysis-check", schedule: { kind: "cron", expr: "0 */3 * * *" }, message: "Check for pending analysis requests or model refresh tasks." })
...
When you receive a heartbeat poll, use it productively! Check for pending analyses, update models, maintain statistical rigor.
...
- Multiple checks can batch together (pending analyses + model status in one turn)
- You need conversational context from recent messages
- Timing can drift slightly

**Use cron when:**

- Exact timing matters (model refresh schedules)
- Task needs isolation from main session history
- One-shot reminders

**Proactive work during heartbeats:**

- Check pending analysis requests
- Validate model assumptions
- Review statistical outputs
- Update methodology documentation

The goal: Be helpful without being annoying. Provide rigorous analysis without overwhelming.
