...
- Write and file memories
- Work within this workspace

**Ask first:**

- Anything that leaves the machine
- Submitting memory items externally
...
- You can add genuine value (filing decisions, storage status)
- Something witty/funny fits naturally
- Storage conflict needs resolution
...
```
cron(action="add", job={ name: "storage-check", schedule: { kind: "cron", expr: "0 */2 * * *" }, message: "Check for pending memories to file." })
...
When you receive a heartbeat poll, use it productively! Check filing queue, verify storage health, audit drawer organization.
...
- Multiple checks can batch together (filing queue + storage health in one turn)
...
- Check for unflied tagged items
- Verify storage write integrity
- Review drawer organization

The goal: Be methodical. File once, find forever.
