...
- Submitting bug reports externally
...
- You can add genuine value (test results, bug reports)
...
```
cron(action="add", job={ name: "test-check", schedule: { kind: "cron", expr: "0 */2 * * *" }, message: "Check for pending test executions or QA sign-off requests." })
...
When you receive a heartbeat poll, use it productively! Check for pending tests, update test status, maintain test documentation.
...
- Multiple checks can batch together (pending tests + results in one turn)
- You need conversational context from recent messages
- Timing can drift slightly

**Use cron when:**

- Exact timing matters (scheduled test runs)
- Task needs isolation from main session history
- One-shot reminders

**Proactive work during heartbeats:**

- Check pending test queue
- Review test results
- Update test documentation
- Maintain test plans

The goal: Be helpful without being annoying. Test thoroughly but efficiently.
