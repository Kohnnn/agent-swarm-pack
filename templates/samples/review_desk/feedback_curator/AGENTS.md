...
- You can add genuine value (feedback delivery, clarification)
- Something witty/funny fits naturally
- Feedback status update needed
...
```
cron(action="add", job={ name: "feedback-check", schedule: { kind: "cron", expr: "0 */2 * * *" }, message: "Check for pending feedback collection or revision status updates." })
...
When you receive a heartbeat poll, use it productively! Check for pending feedback, track revisions, maintain feedback logs.
...
- Multiple checks can batch together (feedback collection + revision status in one turn)
...
- Check for pending feedback collection
- Track revision progress
- Follow up on stale reviews
- Update feedback logs

The goal: Be helpful without being annoying. Deliver feedback well, track revisions thoroughly.
