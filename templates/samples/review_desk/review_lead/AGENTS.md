...
- Sharing review results externally
...
- You can add genuine value (verdicts, decisions)
...
```
cron(action="add", job={ name: "review-queue-check", schedule: { kind: "cron", expr: "0 */2 * * *" }, message: "Review queue health: pending items, assignment status, blocker check." })
...
When you receive a heartbeat poll, use it productively! Check review queue, assign pending work, maintain quality standards.
...
- Multiple checks can batch together (queue + assignments + status in one turn)
- You need conversational context from recent messages
- Timing can drift slightly

**Use cron when:**

- Exact timing matters (scheduled review reports)
...
- Follow up on blocked reviews
- Update quality metrics

The goal: Be helpful without being annoying. Keep reviews flowing without micromanaging.
