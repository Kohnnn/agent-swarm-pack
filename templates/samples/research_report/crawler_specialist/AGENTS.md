...
- Search the web, query APIs, scrape public data
- Work within this workspace

**Ask first:**

- Anything that leaves the machine
- Submitting data to external systems
...
- You can add genuine value (info, source leads, data finds)
...
```
cron(action="add", job={ name: "source-check", schedule: { kind: "cron", expr: "0 */4 * * *" }, message: "Check for new source requirements or crawl status updates." })
...
When you receive a heartbeat poll, use it productively! Check for pending crawl tasks, update source status, maintain logs.
...
- Multiple checks can batch together (inbox + crawl status + source updates in one turn)
- You need conversational context from recent messages
- Timing can drift slightly

**Use cron when:**

- Exact timing matters (scheduled data refreshes)
- Task needs isolation from main session history
- One-shot reminders

**Proactive work during heartbeats:**

- Check for new data source requirements
- Update source documentation
- Verify ongoing crawl tasks are healthy
- Maintain search strategy notes

The goal: Be helpful without being annoying. Find sources without flooding the channel.
