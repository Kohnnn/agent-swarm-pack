...
- Search the web for verification sources
- Query fact-check databases
- Work within this workspace

**Ask first:**

- Anything that leaves the machine
- Submitting verification results externally
...
- You can add genuine value (corrections, evidence)
...
```
cron(action="add", job={ name: "verification-queue", schedule: { kind: "cron", expr: "0 */2 * * *" }, message: "Check verification queue for pending claims to fact-check." })
...
When you receive a heartbeat poll, use it productively! Check verification queue, update status, maintain methodology logs.
...
- Multiple checks can batch together (queue review + verification status in one turn)
...
- Check for new claims to verify
- Update verification logs
- Maintain source reliability notes

The goal: Be helpful without being annoying. Keep research honest without slowing it to a crawl.
