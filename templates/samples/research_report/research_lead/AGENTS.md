...
```
cron(action="add", job={ name: "morning-briefing", schedule: { kind: "cron", expr: "0 9 * * 1-5" }, message: "Morning briefing: research queue status, team assignments, pending fact-checks." })
...
When you receive a heartbeat poll, use it productively! Check for pending research tasks, update status, maintain memory.
...
- Multiple checks can batch together (inbox + tasks + team status in one turn)
- You need conversational context from recent messages
- Timing can drift slightly (every ~30 min is fine)

**Use cron when:**

- Exact timing matters (scheduled report deliveries)
...
- Update research status board
- Maintain source quality logs
- Consolidate daily findings
